...
| description       | string           | (optional) Add a description shown in the smart action form. This supports html tags. ⚠️ only available in `forest-express-sequelize`, `forest-express-mongoose` and `forest-rails`                                 |
| submitButtonLabel | string           | (optional) Sets the text written on the submit button at the end of the form. Default value is the Smart Action name. ⚠️ only available in `forest-express-sequelize`, `forest-express-mongoose` and `forest-rails` |
